...
*/
contract Registry_1 is Ownable {
    error RouteNotFound();
    error InvalidAmount();
    ...
    */
    ...
    modifier onlyExistingRoute(uint256 _routeId) {
        if(routes[_routeId].route == address(0))
        {
    ...
    */
        ...
        payable
    {
        ...
        if(_userRequest.bridgeRequest.id == 0)
        {
        ...
        // make sure bridge input is provided
         ...
         if(_userRequest.bridgeRequest.inputToken == address(0))
         {
        ...
        if( middlewareInfo.route == address(0) && !middlewareInfo.isEnabled && !middlewareInfo.isMiddleware)
        {
        ...
        if(bridgeInfo.route == address(0) && !bridgeInfo.isEnabled && bridgeInfo.isMiddleware)
        {
            revert RouteNotAllowed();
        }
